  ...
  async function loadFromDB() {
    try {
      const { data, error } = await supabase
        ...
        .select("id, match_date, competition, partido, updated_at")
  ...
  async function onAutoSave(idx) {
    ...
    try {
      if (dbEnabledRef.current) {
        if (r.id) {
          const { error } = await supabase.from("matches_finalizados").update(payload).eq("id", r.id);
          if (error) throw error;
        } else {
          const { data, error } = await supabase.from("matches_finalizados").insert(payload).select().maybeSingle();
          if (error) throw error;
          setRows((prev) => {
            const next = prev.slice();
