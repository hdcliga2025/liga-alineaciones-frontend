...
export default function PartidosFinalizados() {
  ...
  const [rows, setRows] = useState([]); // {id,equipo1?,equipo2?,partido?,match_date,competition}
  ...
  // separa equipo1/equipo2 se vienen juntos en 'partido'
  ...
  async function onSave(i){
    ...
    try {
      ...
      const r = rows[i];
      if (!r) return;
      // require 3 campos: partido, data, competición
        ...
        {view.map((r, i) => {
          ...
          return (
            <article key={r.id || `n-${i}`} style={{ ...CARD, ...(savedStyle||{}) }}>
              {/* Fila 1: número + Equipo1 vs Equipo2 (editamos sobre partido) */}
              <div style={ROW1}>
                ...
                <div style={TEAMLINE}>
                  <input
                    ...
                    value={t1}
                    onInput={(e)=> editable && setLocal(i, { partido: joinTeams(e.currentTarget.value.toUpperCase(), t2.toUpperCase()), __saved:false })}
                  ...
                  <input
                    style={INPUT_TEAM(editable)}
                    value={t2}
                    onInput={(e)=> editable && setLocal(i, { partido: joinTeams(t1.toUpperCase(), e.currentTarget.value.toUpperCase()), __saved:false })}
              ...
              {/* Fila 2: Data + Competición + accións (VER e GARDAR) */}
              <div style={ROW2}>
                <input
                ...
                <div style={ACTIONS}>
                  {/* Ver (leva a unha pantalla futura; temporal: ProximoPartido) */}
                  <button
                    type="button"
                    title="Revisar"
                    style={ICONBTN()}
