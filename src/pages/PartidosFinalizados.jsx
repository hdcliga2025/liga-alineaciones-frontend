...
const parsePartido = (s="") => {
  const a = String(s).split(/vs/i);
  return { home:(a[0]||"").trim().toUpperCase(), away:(a[1]||"").trim().toUpperCase() };
};
/* Ocultar dd/mm/aa cando baleiro (webkit) */
const DATE_CSS = `
...
export default function PartidosFinalizados() {
  ...
  // Auto-archivo (sen requirir admin; confíase en políticas RLS)
  useEffect(() => {
    let t = null;
  ...
  async function upsertRow(row, patch) {
    ...
    if (patch.ymd) {
      ...
      payload.match_iso  = `${patch.ymd}T00:00:00`;
    }
    ...
    if (row?.id) await supabase.from("matches_finalizados").update(payload).eq("id", row.id);
    else await supabase.from("matches_finalizados").insert(payload);
    await loadList();
  }
    ...
    <main style={WRAP}>
      ...
      <section style={CARD}>
        ...
        <div style={{ ...GRID, borderTop:"1px solid #0ea5e9", borderBottom:"1px solid #0ea5e9" }}>
          ...
          {headCell(<span>PARTIDO</span>)}
          {headCell(<span>COMPETICIÓN</span>)}
            ...
            <div key={r?.id || `p-${i}`} style={ROW}>
              ...
              {bodyCell(
                <input
                  ...
                  style={inputBase}
                  value={ymd}
                  onInput={(e)=>upsertRow(r, { ymd: e.currentTarget.value })}
                />, 1, last
              )}

              {bodyCell(
                <div style={{ display:"grid", gridTemplateColumns:"1fr auto 1fr", gap:8, alignItems:"center" }}>
                  <input style={inputTeam} defaultValue={home} placeholder="EQUIPO 1" onBlur={(e)=>upsertRow(r,{ home:e.currentTarget.value, away })}/>
                  <span style={{ fontWeight:800, color:"#0f172a" }}>vs</span>
                  <input style={inputTeam} defaultValue={away} placeholder="EQUIPO 2" onBlur={(e)=>upsertRow(r,{ home, away:e.currentTarget.value })}/>
                </div>, 2, last
              )}

              {bodyCell(
                <select style={selectBase} value={r?.competition || ""} onInput={(e)=>upsertRow(r,{ comp:e.currentTarget.value })}>
                  <option value=""></option>
              ...
              {bodyCell(
                <div style={{ display:"flex", justifyContent:"center" }}>
                  <button type="button" style={BTN_ICON} title="Ver detalle" onClick={() => route("/proximo-partido")} disabled={!r?.id}>
