...
const S = {
  ...
  info: { margin:"0 0 14px", padding:"10px 12px", borderRadius:10, background:"#eef2ff", border:"1px solid #c7d2fe", color:"#1e3a8a", fontSize:14, fontWeight:600 },
  resumen: {
...
export default function ConvocatoriaProximo() {
  ...
  useEffect(() => {
    (async () => {
      // Header: preferimos o primeiro vindeiro; se non, next_match
      ...
      if (!ids.length) { setConvocados([]); setLoading(false); return; }
      ...
      // Conservar orden de publicación segundo ids, pero con datos
      const byId = new Map((js||[]).map(j => [j.id, j]));
      const ordered = ids.map(id => byId.get(id)).filter(Boolean);
      setConvocados(ordered);
      setLoading(false);
  ...
  return (
    <main style={S.wrap}>
      ...
      <p style={S.sub}>Listado comunicado polo club para o seguinte encontro.</p>
      ...
      <div style={S.info}>
        {convocados.length} xogadores convocados. (POR / DEF / CEN / DEL)
      </div>
