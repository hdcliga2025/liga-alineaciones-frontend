...
const COL_BORDER = "1px solid rgba(15,23,42,.22)"; // columna
const BTN_ICON = { display:"inline-grid", placeItems:"center", width:36, height:36, border:"1px solid #e5e7eb", borderRadius:10, background:"#fff", boxShadow:"0 2px 8px rgba(0,0,0,.06)", cursor:"pointer" };
...
export default function VindeirosPartidos() {
  ...
  const [rows, setRows] = useState(
    Array.from({ length: 10 }, () => ({ date: "", home: "", away: "", comp: "" }))
  );

  const onChange = (i, field, value) => {
    setRows(prev => {
      const nx = prev.slice();
      nx[i] = { ...nx[i], [field]: value };
      return nx;
    });
  };
        ...
        <div style={{ ...GRID, borderTop:"1px solid #e5e7eb", borderBottom:"1px solid #e5e7eb", background:"#f8fafc" }}>
          ...
          {headCell(
            <div style={{ display:"flex", alignItems:"center", gap:8 }}>
              {/* Balón bonito e algo máis grande */}
          ...
          <div key={i} style={ROW}>
            ...
            {bodyCell(
              <div style={{ display:"flex", justifyContent:"center" }}>
                <button type="button" style={{ ...BTN_ICON, width:40, height:40 }} disabled>
                  <svg width="22" height="22" viewBox="0 0 24 24" fill="none" aria-hidden="true">
