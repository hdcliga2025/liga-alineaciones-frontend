...
function enrichPlayer(p = {}) {
  ...
  return { id:p.id, nombre:(nameFile || p.nombre || "").trim(), dorsal:dorsalFile ?? p.dorsal ?? null, pos:(posFile || p.pos || "").toUpperCase() };
}
...
const S = {
  ...
  resumen: {
    ...
    background:"linear-gradient(180deg,#dcfce7,#bbf7d0)", color:"#064e3b",
...
export default function ResultadosUltimaAlineacion() {
    ...
    (async () => {
      try {
        ...
        const [ofiQ, allUserPicks, profsQ, jugadoresQ, mvQ] = await Promise.all([
          ...
          supabase.from("public_profiles_cache").select("user_id, display_name"),
        ...
        const nameMap = new Map((profsQ.data || []).map(p => [p.user_id, p.display_name]));
        ...
        for (const [uid, { picked, last }] of byUser.entries()) {
          ...
          const shownName = nameMap.get(uid) || "xogador/a";
  ...
  const renderLineup = useCallback((list=[]) => (
    <span>
      {list.map((p, idx) => (
        <span key={p.id || idx}>
          <span style={{ whiteSpace:"nowrap", display:"inline-block", ...(p.isHit ? S.hit : S.miss) }}>
    ...
    <main style={S.wrap}>
      ...
      {header && (
        <section style={S.resumen}>
          ...
          <p style={S.resumeMuted}>Participantes: <strong>{rows.length}</strong></p>
        </section>
      )}
