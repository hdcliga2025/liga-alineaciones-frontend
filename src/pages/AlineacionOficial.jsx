...
function finalFromAll(p = {}) {
  ...
  return { dorsal: dorsalFile ?? (p.dorsal ?? null), pos: (posFile || "").toUpperCase(), nombre: (nameFile || p.nombre || "").trim() };
}

/* Ajuste de texto en móvil (2 líneas) */
function useFitText2Lines(ref, { min=11, max=14, lineHeight=1.2, maxLines=2, initial=13.5, deps=[] } = {}) {
  const [fontSize, setFontSize] = useState(initial);
  useEffect(() => {
    const el = ref.current; if (!el) return;
    el.style.whiteSpace = "normal"; el.style.overflow = "hidden"; el.style.display = "block";
    el.style.textOverflow = "clip"; el.style.lineHeight = String(lineHeight); el.style.wordBreak = "break-word";
    let low = min, high = max, best = Math.max(min, Math.min(max, initial));
    const fits = (px) => { el.style.fontSize = px + "px"; const one = px * lineHeight; const maxH = one * maxLines + 0.5; return el.scrollHeight <= maxH && el.clientHeight <= maxH; };
    if (fits(best)) low = best; else high = best;
    while (high - low > 0.25) { const mid = (low + high)/2; if (fits(mid)) low = mid; else high = mid; }
    best = Math.floor(low*10)/10; el.style.fontSize = best + "px"; setFontSize(best);
    if (!fits(min)) { el.style.display = "-webkit-box"; el.style.webkitBoxOrient = "vertical"; el.style.webkitLineClamp = String(maxLines); el.style.textOverflow = "ellipsis"; }
    let raf=0; const onR=()=>{ cancelAnimationFrame(raf); raf=requestAnimationFrame(()=>setFontSize((s)=>s)); };
    window.addEventListener("resize", onR); document.addEventListener("visibilitychange", onR);
...
const S = {
  ...
  resumen: {
    ...
    color: "#7f1d1d",
    boxShadow: "inset 0 1px 0 rgba(255,255,255,.75), 0 12px 28px rgba(239,68,68,.22), 0 2px 0 rgba(239,68,68,.18)",
  },
  ...
  frame: (isMobile) => ({ width: "100%", height: isMobile ? 172 : 320, borderRadius: 12, overflow: "hidden", background: "#ffffff", display: "grid", placeItems: "center", border: "1px solid #e5e7eb", position: "relative" }),
  ...
  btnLoad: (isMobile) => ({
    width: "100%", padding: isMobile ? "9px 12px" : "12px 16px", borderRadius: 12,
    background: "linear-gradient(180deg,#ffe9e9,#fca5a5)",
    border: "1px solid #ef4444", color: "#7f1d1d", fontWeight: 800, fontSize: isMobile ? 14 : 16, letterSpacing: 0.4, cursor: "pointer",
    boxShadow: "inset 0 1px 0 rgba(255,255,255,.8), 0 4px 10px rgba(239,68,68,.22), 0 1px 0 rgba(239,68,68,.25)",
  ...
  btnTrash: {
    ...
    color: "#7f1d1d", border: "1px solid #ef4444", cursor: "pointer", display: "grid", placeItems: "center",
    boxShadow: "inset 0 1px 0 rgba(255,255,255,.9), 0 3px 8px rgba(239,68,68,.18), 0 1px 0 rgba(239,68,68,.18)",
    transition: "transform .06s ease, box-shadow .2s ease", willChange: "transform",
  }),
  btnBottom: (isMobile) => ({
    width: "100%", padding: isMobile ? "9px 12px" : "12px 16px", borderRadius: 12,
    background: "linear-gradient(180deg,#ffe9e9,#fca5a5)",
    border: "1px solid #ef4444", color: "#7f1d1d", fontWeight: 800, fontSize: isMobile ? 14 : 16, letterSpacing: 0.4, cursor: "pointer", marginTop: 14,
    boxShadow: "inset 0 1px 0 rgba(255,255,255,.8), 0 4px 10px rgba(239,68,68,.22), 0 1px 0 rgba(239,68,68,.25)",
    transition: "transform .06s ease, box-shadow .2s ease", willChange: "transform",
  }),
  ...
  upValue: (blinkOn) => ({ margin: "2px 0 0", font: "700 15px/1.35 Montserrat,system-ui,sans-serif", color: blinkOn ? "#7f1d1d" : "#0f172a", transition: "color .25s ease" }),

  okBadgeDesktop: { position: "absolute", top: 8, right: 8, background: "rgba(56,189,248,.95)", color: "#fff", borderRadius: 999, padding: "3px 7px", font: "700 12px/1 Montserrat,system-ui,sans-serif", boxShadow: "0 2px 8px rgba(56,189,248,.35)", userSelect: "none", pointerEvents: "none" },
  okBadgeMobile: { position: "absolute", top: 3, right: 3, background: "rgba(56,189,248,.98)", color: "#fff", borderRadius: 999, padding: "2px 6px", font: "700 10.5px/1 Montserrat,system-ui,sans-serif", boxShadow: "0 1px 4px rgba(56,189,248,.35)", userSelect: "none", pointerEvents: "none" },
};

function NameMobileTwoLines({ text }) {
  const ref = useRef(null);
  const { fontSize } = useFitText2Lines(ref, { deps: [text] });
  return <p ref={ref} style={{ ...S.nameMobileBase, fontSize, maxHeight: fontSize ? `${fontSize*1.2*2 + .5}px` : undefined }}>{text}</p>;
}
...
export default function AlineacionOficial() {
  ...
  // Parpadeo cada 1s
  useEffect(() => { const id = setInterval(() => setBlinkOn(v => !v), 1000); return () => clearInterval(id); }, []);
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data: top } = await supabase.from("matches_vindeiros").select("id,equipo1,equipo2,match_iso").order("match_iso", { ascending: true }).limit(1).maybeSingle();
      if (top?.match_iso) {
  ...
  async function preloadOfficial(iso) {
    try {
      const { data: ofi } = await supabase.from("alineacion_oficial").select("jugador_id,updated_at").eq("match_iso", iso);
      if (ofi && ofi.length) {
      ...
      } else { setSel(new Set()); setLastSavedAt(null); }
    } catch {}
  }

  const grouped = useMemo(() => {
    const g = { POR: [], DEF: [], CEN: [], DEL: [] };
    for (const p of players || []) { const { pos } = finalFromAll(p); if (pos && g[pos]) g[pos].push(p); }
    return g;
  }, [players]);
  ...
  function playBeep() {
    try {
      ...
      o.connect(g); g.connect(ctx.destination); o.start(); o.stop(ctx.currentTime + 0.14);
    } catch {}
  }

  function togglePick(id) {
    setSel(prev => { const n = new Set(prev); if (n.has(id)) n.delete(id); else { if (n.size >= max11) return prev; n.add(id); } return n; });
    setLastCounterId(id); playBeep();
  }
  ...
  async function resolveEncuentroId(iso) {
    ...
    if (a?.data?.id) return a.data.id; return null;
  }

  async function loadOfficial() {
    if (!isAdmin) return showToast("Só admins poden gardar a aliñación oficial.");
    if (sel.size !== 11) return showToast("Escolle 11 xogadores.");
    if (!header?.match_iso) return showToast("Falta o partido de referencia.");
    ...
    try {
      ...
      if (invalid.length) {
        ...
        showToast(`IDs non-UUID en xogadores: ${names.join(", ")}`); setSaving(false); return;
      }

      const check = await supabase.from("jugadores").select("id").in("id", ids);
      const okSet = new Set((check.data || []).map((r) => r.id));
      if (okSet.size !== ids.length) { const missing = ids.filter((id) => !okSet.has(id)); showToast(`Xogadores inexistentes: ${missing.join(", ")}`); setSaving(false); return; }
      ...
      setLastSavedAt(now); showToast("Aliñación oficial gardada.");
    } catch (e) {
    ...
    <main style={S.wrap}>
                ...
                return (
                  <article key={p.id} style={S.card(picked)} onClick={() => togglePick(p.id)}>
                    <div style={S.frame(isMobile)}>
                      ...
                      {picked && <span style={isMobile ? S.okBadgeMobile : S.okBadgeDesktop}>OK</span>}
                    </div>
      ...
      {toast && (
        <div role="status" aria-live="polite" style={{
          ...
          background: "#0ea5e9", color: "#fff", padding: "10px 16px", borderRadius: 12,
          boxShadow: "0 10px 22px rgba(2,132,199,.35)", fontWeight: 700, maxWidth: "92vw",
